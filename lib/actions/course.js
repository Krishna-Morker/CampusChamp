...
export const Addcourse = async (req) => {
  ...
  export const Addstudent = async (req) => {
    ...
    try {
      ...
      //  const proffesor=await User.findOne({ _id: course.ProfID });
        ...
        await pusher.trigger(
          `course-${course._id}-notifications`, // Targeting the course-specific channel
          'student-joined', // Event name
          {
            message: `${use.username} has joined the course ${course.CourseName}`,
          }
        );
  ...
  export const Getstudent = async (req) => {
    ...
    try {
      ...
      // Use Promise.all to resolve all asynchronous operations in parallel
